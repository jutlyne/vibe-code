<template>
  <div class="space-y-4">
    <button
      type="button"
      class="w-full border border-dashed p-8 text-center transition-colors cursor-pointer"
    ...
    >
      ...
      <p class="text-[10px] font-light tracking-wide text-stone-500 mt-1">Có thể chọn nhiều ảnh cùng lúc</p>
    </button>
    ...
    <div v-if="localImages.length > 0" class="grid grid-cols-2 sm:grid-cols-3 gap-3">
      ...
      >
        <img
          ...
          :alt="`Ảnh yêu thích ${idx + 1}`"
        />
        <div class="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-colors" />
        <button
          class="absolute top-2 right-2 w-9 h-9 rounded-full bg-red-500 text-white text-xs flex items-center justify-center opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity hover:bg-red-600"
          type="button"
          aria-label="Xóa ảnh"
